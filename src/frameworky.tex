...
it can be done using \Code{DataSource} in \Code{javax.sql} package.
Using \Code{DataSource} is now the preferred way of connecting to database,
but many applications continue to use old-fashioned \Code{DriverManager}.
...
logic (how result row should be transformed to \Code{DatabaseValue} object)
...
one can define a SQL fragment and reuse it in more queries.
...
\begin{enumerate}
  ...
  \item \textbf{Use concrete values.} \\
    The analysis should know the concrete values of Java primitives and \Code{String}s.
