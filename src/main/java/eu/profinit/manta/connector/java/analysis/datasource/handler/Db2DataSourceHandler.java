...
import static eu.profinit.manta.connector.java.analysis.datasource.handler.DataSourceClassWrappers.DB2_BASE_DATA_SOURCE;

public class Db2DataSourceHandler implements CallHandler {
    private final CallHandlers handlers;

    public Db2DataSourceHandler(@Nonnull final ClassMethodCache classMethodCache) {
        Validate.notNull(classMethodCache);

        this.handlers = new ClassCallHandlers(this,
                DB2_BASE_DATA_SOURCE,
                new CallHandler() {
                    @Override
    ...
    /**
     * See com.ibm.db2.jcc.DB2SimpleDataSource#setUser(String)
     */
    @Nonnull
