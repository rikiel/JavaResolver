...
import static eu.profinit.manta.connector.java.analysis.datasource.handler.DataSourceClassWrappers.POSTGRESQL_BASE_DATA_SOURCE;

public class PostgreSqlDataSourceHandler implements CallHandler {
    private final CallHandlers handlers;

    public PostgreSqlDataSourceHandler(@Nonnull final ClassMethodCache classMethodCache) {
        Validate.notNull(classMethodCache);

        this.handlers = new ClassCallHandlers(this,
                POSTGRESQL_BASE_DATA_SOURCE,
                new CallHandler() {
                    @Override
    ...
    /**
     * See org.postgresql.ds.common.BaseDataSource#setUrl(String)
     */
    @Nonnull
    private MethodEffectsDescriptionBuilder handleSetUrl(@Nonnull final MethodCallDescriptionImpl methodCallDescription) {
        return MethodEffectsDescriptionBuilder.propagateFlow(methodCallDescription, TO_RECEIVER)
                .dataEndpointFlowInfoBuilder().objectFlowInfoBuilder(TO_RECEIVER)
    ...
    /**
     * See org.postgresql.ds.common.BaseDataSource#setUser(String)
     */
    @Nonnull
